 ...
 * 用于用户的Adapter
 */
public class ContactListArrayAdapter extends BaseAdapter implements TweetAdapter, OnCompletionListener, OnErrorListener, OnClickListener {
	...
	private int mType=1; //0: chat list; 1: contacts list
...
//    };
	...
	private ImageLoaderCallback callback = new ImageLoaderCallback() {
		...
		public void refresh(String url, Bitmap bitmap) {
			ContactListArrayAdapter.this.refresh();
		}
	};

	public ContactListArrayAdapter(Context context, int type) {
...
//		boolean useProfileImage = pref.getBoolean(Preferences.USE_PROFILE_IMAGE, true);
		...
		final Friend chat = mFriends.get(position);
		...
		if(mType==1) {
			...
			holder.contact_info.setVisibility(View.VISIBLE);
	...
	public void refresh(ArrayList<Friend> friends) {
		mFriends = (ArrayList<Friend>) friends.clone();
