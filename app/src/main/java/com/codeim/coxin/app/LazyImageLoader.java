...
public class LazyImageLoader {
	...
	public Bitmap get(String url, ImageLoaderCallback callback) {
		...
		if (mImageManager.isContains(url)) {
			Log.v(TAG, "Bitmap get(String, ImageLoaderCallback), Image 1st in cache: " + url);
			bitmap = mImageManager.get(url);
		} else {
			Log.v(TAG, "Bitmap get(String, ImageLoaderCallback), Image 1st not in cache, to download: " + url);
		...
		public void run() {
			try {
				while (!mTaskTerminated) {
					...
					//Bitmap bitmap = mImageManager.safeGet(url); //for debug
	...
	public static class CallbackManager {
		...
		public void put(String url, ImageLoaderCallback callback) {
			...
			if (!mCallbackMap.containsKey(url)) {
				Log.v(TAG, "url does not exist in callbackManager, add new key list to map");
