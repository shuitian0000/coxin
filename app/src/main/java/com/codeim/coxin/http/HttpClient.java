...
//import com.codeim.coxin.fanfou.RefuseError;
...
public class HttpClient {
	...
	//private static final String SERVER_HOST = "www.imyouliao.com";  // api.fanfou.com
	private static final String SERVER_HOST = "52.33.47.51";//"121.199.73.2";//"webst.sinaapp.com";//"121.199.73.2";  // api.fanfou.com
...
//	private OAuthClient mOAuthClient;
	...
	public HttpClient() {
	    Log.d(TAG, "HttpClient at HttpClient");
		prepareHttpClient();
...
//    }
	...
	private void prepareHttpClient() {
		if (DEBUG) {
		    Log.d(TAG, "HttpClient at prepareHttpClient");
			enableDebug();
		}
...
//		mClient.addResponseInterceptor(gzipResponseIntercepter);
	...
	private void setBasicAuth() {
        ...
        mAuthScope = new AuthScope(SERVER_HOST, AuthScope.ANY_PORT, AuthScope.ANY_REALM);
	...
	public void setCredentials(String username, String password) {
		...
		Log.d(TAG, "username = " + username);
		mClient.getCredentialsProvider().setCredentials(mAuthScope, new UsernamePasswordCredentials(username, password));
		//isAuthenticationEnabled = true;
	...
	public Response post(String url, ArrayList<BasicNameValuePair> params) throws HttpException {
	...
	public Response get(String url, ArrayList<BasicNameValuePair> params, boolean authenticated) throws HttpException {
		return httpRequest(url, params, authenticated, HttpGet.METHOD_NAME);
	}

	public Response get(String url, ArrayList<BasicNameValuePair> params) throws HttpException {
	...
	public Response httpRequest(String url, ArrayList<BasicNameValuePair> postParams, File file,
		...
		Log.d(TAG, "after DEBUG");
		
		authenticated = true;//ywwang 20140524
...
//				mOAuthClient.signRequest(method);
			...
			Log.d(TAG, "before mClient execute");
			response = mClient.execute(method, localcontext);    // mClient.execute(method, localcontext);
			Log.d(TAG, "after mClient execute");
...
//			Log.v(TAG, res.asString());
		...
		Log.d(TAG, "complete httpRequest");
		
		return res;
	}
	
	public Response httpRequestImage(String url, ArrayList<BasicNameValuePair> postParams, File file,
			boolean authenticated, String httpMethod) throws HttpException {
		Log.d(TAG, "Sending " + httpMethod + " request to " + url);
		if (TwitterApplication.DEBUG) {
			DebugTimer.betweenStart("HTTP");
		...
		// Create POST, GET or DELETE METHOD
		method = createMethodWithImage(httpMethod, uri, file, postParams);
		// Setup ConnectionParams, Request Headers
		SetupHTTPConnectionParams(method);
...
//		}
		...
		Log.d(TAG, "complete httpRequestImage");
	 ...
	 */
	private MultipartEntity createMultipartEntity(String filename, File file, ArrayList<BasicNameValuePair> postParams)
			throws UnsupportedEncodingException {
	...
	private HttpUriRequest createMethod(String httpMethod, URI uri, File file, ArrayList<BasicNameValuePair> postParams) throws HttpException {
		...
		if (httpMethod.equalsIgnoreCase(HttpPost.METHOD_NAME)) {
			...
			try {
				...
				if (null != file) {
					entity = createMultipartEntity("media", file, postParams);  //"photo" "media"
	...
	private HttpUriRequest createMethodWithImage(String httpMethod, URI uri, File file, ArrayList<BasicNameValuePair> postParams) throws HttpException {
		...
		if (httpMethod.equalsIgnoreCase(HttpPost.METHOD_NAME)) {
			...
			try {
				HttpEntity entity = null;
				if (null != file) {
					entity = createMultipartEntity("image", file, postParams);  //"photo" "media"
					post.setEntity(entity);
				} else if (null != postParams) {
					entity = new UrlEncodedFormEntity(postParams, HTTP.UTF_8);
				}
	...
	public static String encodeParameters(ArrayList<BasicNameValuePair> params) throws HttpException {
		...
		for (int j = 0; j < params.size(); j++) {
			...
			try {
				buf.append(URLEncoder.encode(params.get(j).getName(), "UTF-8")).append("=")
