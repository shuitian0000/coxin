...
public class FloorView extends LinearLayout{
    ...
    public void init () {
        if ( null == datas.iterator () ) return ;
        if ( datas.getFloorNum () < 7 ) {
            for (Iterator < Comment > iterator = datas.iterator (); iterator.hasNext ();) {
...
//            int margin = Math.min ( ( count - i - 1 ), 4 ) * density ;
            int margin = Math.min ( ( i ), 4 ) * density ;
...
//                layout.topMargin = Math.min ( ( count - i ), 4 ) * density ;
            	layout.topMargin = Math.min ( ( i ), 4 ) * density ;
            }
